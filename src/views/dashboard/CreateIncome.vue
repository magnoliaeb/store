<template>
  ...
  @onCloseNofity="showNotify = $event" />
    ...
    <h2 class="mb-6 text-md-h4 font-weight-bold">Crear nueva ingreso</h2>
                ...
                >
                 <v-row>
                    ...
                    <v-col cols="12" md="12" lg="6">
                        <v-autocomplete
                            label="Proveedor" :rules="rules.providerRules"
                            :items="providers"
...
export default {
  ...
  methods: {
     async getProvider() {
        ...
        const providers = res.data.records.map((item) => {
            return {
                text: item.name,
                value: item._id
            }
         ...
         if (this.$refs.form.validate() && this.validateDetails()) {
          try {
            const income = { 
             ...
             await http.post("/incomes", income, config);
