...
@onCloseNofity="showNotify = $event" />
    ...
    <h2 class="mb-6 text-md-h4 font-weight-bold">Proveedores</h2>
...
export default {
  ...
  filters: {
   ...
   watch: {
    ...
    'editedItem.num_document': function(value) {
      ...
      const existProvider = this.providers.some((item) => item.num_document.toLowerCase() === value.toLowerCase());

      const providerSelected = this.providers.find((item) => item._id === this.itemSelected)

      if (this.editedIndex > -1 && providerSelected.num_document.toLowerCase() === value.toLowerCase()) return
  ...
  methods: {
    async getProviders() {
      ...
      const res = await http.get("/providers", config);
    ...
    async changeState(item) {
      ...
      if (item.state) {
        await http.put(`/providers/${item._id}/disabled`, {}, config);
        this.providers[index].state = false;
      } else {
        await http.put(`/providers/${item._id}/enabled`, {}, config);
        this.providers[index].state = true;
      }
    },
     editItem(item) {
     ...
     const header = { token: this.$store.getters['auth/token'] };
      ...
      if (this.$refs.form.validate()) {
        ...
        if (this.editedIndex > -1) {
          try {
            await http.put(`/providers/${this.itemSelected}`, this.editedItem, config);
          ...
          try {
            await http.post("/providers", this.editedItem, config);
